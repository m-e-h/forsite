<?php
/**
 * Forsite\Lazyload\Component class
 *
 * @package forsite
 */

namespace Forsite\Lazyload;
...
class Component implements Component_Interface {
	...
	public function action_customize_register_lazyload( WP_Customize_Manager $wp_customize ) {
		$lazyload_choices = [
			'lazyload'    => __( 'Lazy-load on (default)', 'forsite' ),
			'no-lazyload' => __( 'Lazy-load off', 'forsite' ),
		];
		...
		$wp_customize->add_control(
			'lazy_load_media',
			[
				'label'           => __( 'Lazy-load images', 'forsite' ),
				'section'         => 'theme_options',
				'type'            => 'radio',
				'description'     => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'forsite' ),
	...
	public function action_enqueue_lazyload_assets() {
		wp_enqueue_script(
			'forsite-lazy-load-images',
			get_theme_file_uri( '/assets/js/lazyload.min.js' ),
			[],
			forsite()->get_asset_version( get_theme_file_path( '/assets/js/lazyload.min.js' ) ),
			false
		);
		wp_script_add_data( 'forsite-lazy-load-images', 'defer', true );
		wp_script_add_data( 'forsite-lazy-load-images', 'precache', true );
	}
